...
proc ptable[NPROC];             // arra y of process descriptors
...
void process_setup(pid_t pid, const char* program_name) {
    ...
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        for (uintptr_t a = round_down(seg.va(), PAGESIZE);
            ...
            uintptr_t loc2 = (uintptr_t) kalloc(sizeof(size_t));
            memory_map(ptable[pid].pagetable, a, loc2, 0x7UL);
        }
    }

    // initialize data in loadable segments
    for (auto seg = pgm.begin(); seg != pgm.end(); ++seg) {
        void *pa = memory_virtual_to_physical(ptable[pid].pagetable ,(uintptr_t) seg.va());
    ...
    uintptr_t loc3 = (uintptr_t) kalloc(PAGESIZE);
    memory_map(ptable[pid].pagetable, stack_addr, loc3, 0x7UL);
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    // uintptr_t loc = (uintptr_t) kalloc(PAGESIZE);
    uintptr_t loc = (uintptr_t) kalloc(PAGESIZE);
    if (loc == (uintptr_t) nullptr) {return -1;}
    memory_map(current->pagetable, addr, loc, 0x7UL);
    void *pa = memory_virtual_to_physical(current->pagetable, addr);
